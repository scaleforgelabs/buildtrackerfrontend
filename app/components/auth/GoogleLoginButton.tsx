...
export default function GoogleLoginButton({ label = 'Login with Google', onError }: GoogleLoginButtonProps) {
    ...
    useEffect(() => {
        ...
        // Render Google's button in a hidden div so we can trigger it from our custom button
    ...
    const handleClick = () => {
        ...
        const googleBtn = hiddenBtnRef.current?.querySelector('div[role="button"]') as HTMLElement;
        if (googleBtn) {
            googleBtn.click();
        } else {
    ...
    return (
        <>
            ...
            <div ref={hiddenBtnRef} style={{ position: 'absolute', opacity: 0, pointerEvents: 'none', height: 0, overflow: 'hidden' }} />
