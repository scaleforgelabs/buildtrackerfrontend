...
import { Search, Bell, Moon, Settings, Sun, LogOut, CheckSquare, BookOpen, Users, Link2, Plug, FileText, FolderOpen, Loader2, X } from "lucide-react";
...
const TYPE_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  ...
  integration: { label: 'Integration', icon: Plug, color: 'text-purple-600 bg-purple-50' },
  team: { label: 'Team', icon: Users, color: 'text-orange-600 bg-orange-50' },
  quicklink: { label: 'Quick Link', icon: Link2, color: 'text-cyan-600 bg-cyan-50' },
  log: { label: 'Activity', icon: FileText, color: 'text-gray-600 bg-gray-50' },
  notification: { label: 'Notification', icon: Bell, color: 'text-red-600 bg-red-50' },
  file: { label: 'File', icon: FolderOpen, color: 'text-amber-600 bg-amber-50' },
};
      ...
      <div className="flex items-center justify-between rounded-2xl bg-muted px-3 md:px-6 py-3 gap-2">
        ...
        <div className="relative flex-1 max-w-xl" ref={dropdownRef}>
          <div className="flex items-center gap-2 bg-card rounded-full px-4 py-3">
            ...
            />
            {query && (
              <button onClick={clearSearch} className="flex-shrink-0 p-0.5 rounded-full hover:bg-muted">
              ...
              {loading && results.length === 0 ? (
                ...
                <div className="py-2">
                  {Object.entries(groupedResults).map(([type, items]) => {
                    const config = TYPE_CONFIG[type] || { label: type, icon: FileText, color: 'text-gray-600 bg-gray-50' };
                        ...
                        {items.slice(0, 5).map((result) => (
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              ...
                              {result.content && (
                                <p className="text-xs text-muted-foreground truncate mt-0.5">{result.content}</p>
                              )}
                            </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <button
            ...
            className="h-10 w-10 md:h-12 md:w-12 rounded-full bg-card border border-border flex items-center justify-center hover:bg-muted transition hover:bg-red-50 hover:border-red-200 group"
            title="Logout"
          >
          ...
          <div className="flex items-center gap-2 pl-2">
            <UserAvatar user={user} size={48} className="md:w-12 md:h-12 w-10 h-10" />
