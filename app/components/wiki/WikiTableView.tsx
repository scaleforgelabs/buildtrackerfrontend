...
type WikiTableItem = {
    ...
    modifiedAt: string;
    size?: number;
    owner?: { first_name?: string; last_name?: string; email?: string; avatar?: string; name?: string };
...
function ItemIcon({ item }: { item: WikiTableItem }) {
    ...
    if (svgSrc) {
        return <Image src={svgSrc} alt="" width={32} height={32} unoptimized />;
    }
    return (
        <div className="h-8 w-8 shrink-0 rounded bg-muted flex items-center justify-center text-xs font-bold text-muted-foreground">
    ...
    return (
        <div className="flex items-center">
            <div className="flex -space-x-2">
                {visible.map((m, i) => (
                    <UserAvatar key={i} user={{ avatar: m.avatar }} size={28} className="border-2 border-background" />
                ))}
            </div>
            {extra > 0 && (
...
function TableRow({ item, onRenameSuccess }: { item: WikiTableItem; onRenameSuccess?: () => void }) {
    ...
    const handleRenameSubmit = async () => {
        if (!renameVal.trim() || renameVal === item.name) { setRenaming(false); return; }
        try {
            if (item.kind === "folder") {
                await filesService.renameFolder(currentWorkspace?.id!, item.id, { name: renameVal });
    ...
    const ownerLabel = item.owner
        ? (`${item.owner.first_name ?? ""} ${item.owner.last_name ?? ""}`.trim() || item.owner.name || item.owner.email || "–")
        : "–";
                ...
                <div className="flex items-center gap-3">
                    ...
                    {renaming ? (
                        <input
                            ...
                            onKeyDown={(e) => { if (e.key === "Enter") handleRenameSubmit(); e.stopPropagation(); }}
                            className="text-sm font-medium text-foreground bg-muted px-2 py-1 rounded outline-none border border-primary"
                        />
                    ) : (
                        <span className="text-sm font-medium text-foreground">
                            {item.name}
                            {item.kind === "folder" && item.itemCount !== undefined && (
                                <span className="ml-1 text-xs text-muted-foreground">({item.itemCount})</span>
                            )}
                        </span>
                    )}
                </div>
            ...
            <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                <div className="relative inline-block">
                    <button
                        ...
                        className="flex h-8 w-8 items-center justify-center rounded-full hover:bg-muted transition-colors opacity-0 group-hover:opacity-100"
                    >
                    ...
                    {menuOpen && (
                        <>
                            ...
                            <div className="absolute right-0 z-20 mt-1 w-40 rounded-xl border bg-card shadow-lg py-1 text-sm">
                                {item.kind === "folder" && (
                                    <button
                                        onClick={() => { item.onOpen?.(); setMenuOpen(false); }}
...
function TableSection({ section, showMoreCount = 7 }: { section: WikiTableSection; showMoreCount?: number }) {
    const [expanded, setExpanded] = useState(false);
    const visible = expanded ? section.items : section.items.slice(0, showMoreCount);
    ...
    return (
        <div className="space-y-0">
            ...
            <div className="flex items-center gap-2 pb-3">
                ...
                <span className="text-primary text-base font-normal">{section.count}</span>
            </div>
                ...
                <table className="w-full">
                    <thead>
                        <tr className="border-b border-border/50">
                            ...
                            <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">Last modified</th>
