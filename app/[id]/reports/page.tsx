...
import { AnalyticsService, DashboardStats, DashboardCharts, PerformanceAnalytics } from "@/app/services/analytics";
...
export default function ReportsPage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const chartParams = { Period: period, DateFrom: dateFrom, DateTo: dateTo };
    ...
    const displayStatusData = activeStatusData.length > 0 ? activeStatusData : [{ name: "No Data", value: 1, color: "#e2e8f0" }];

    // Prepare member data from API
    const membersData: Member[] = charts?.memberPerformance.map(m => ({
        ...
        assigned: (m.tasks_pending || 0) + (m.tasks_in_progress || 0) + (m.tasks_completed || 0),
    ...
    const handleExportReports = () => {
        ...
        // Create CSV Header
        const headers = ["Name", "Email", "Role", "Assigned Tasks", "Completed Tasks", "Overdue Tasks", "Completion Rate (%)", "Avg Time (Days)"];
        ...
        link.setAttribute("download", `${currentWorkspace?.name?.replace(/ /g, '_') || 'Workspace'}_Productivity_Report_${format(new Date(), 'yyyy-MM-dd')}.csv`);
    ...
    return (
        <div className="p-4 md:p-8 space-y-8 bg-muted min-h-full">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between px-2">
                <div>
                    <h1 className="text-3xl md:text-4xl font-bold text-foreground tracking-tight">Productivity Reports - {currentWorkspace?.name || 'Loading...'}</h1>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-[2rem] bg-blue-600 p-7 text-white flex flex-col justify-between min-h-[160px]">
                    ...
                    <div>
                        ...
                        <p className="text-sm mt-2 opacity-80">{stats?.inProgressTasks || 0} Currently Active</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                ...
                <div className="lg:col-span-4 rounded-[2.5rem] bg-card p-8 border border-border">
                    ...
                    <div className="h-[250px] w-full relative">
                        ...
                        <div className="absolute top-0 left-0 text-xs font-semibold text-muted-foreground">Pending <span className="text-[#F97316]">{statusChartData.find(d => d.name === "Pending")?.value || 0}</span></div>
                        <div className="absolute top-1/2 -right-4 text-xs font-semibold text-muted-foreground -translate-y-1/2">Completed <span className="text-[#22C55E]">{statusChartData.find(d => d.name === "Completed")?.value || 0}</span></div>
                        <div className="absolute bottom-0 left-0 text-xs font-semibold text-muted-foreground">In Progress <span className="text-[#3B82F6]">{statusChartData.find(d => d.name === "In Progress")?.value || 0}</span></div>
                    </div>
                </div>
                    ...
                    <div className="space-y-8">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-xl font-bold">{performance?.averageTaskTime || 0}d</span>
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-xl font-bold">
                                    {stats?.totalMembers ? Math.round(stats.totalTasks / stats.totalMembers) : 0}
                                </span>
                                <div className="bg-green-100 p-1 rounded-md">
                                    <ArrowUpRight className="h-4 w-4 text-green-600" />
                                </div>
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-xl font-bold">{performance?.completionRate ? Math.round(performance.completionRate) : 0}%</span>
                                <div className="bg-green-100 p-1 rounded-md">
                                    <ArrowUpRight className="h-4 w-4 text-green-600" />
                                </div>
                            </div>
                    ...
                    <div className="space-y-6 max-h-[300px] overflow-y-auto pr-2">
                        {performance?.bottlenecks && performance.bottlenecks.length > 0 ? (
                            performance.bottlenecks.map((item, idx) => (
                                <div key={idx} className="flex items-start justify-between group">
                                    <div className="flex gap-4">
                                        <div className={`w-1 self-stretch rounded-full ${item.severity === 'high' ? 'bg-red-500' : 'bg-orange-400'}`} />
                                        <div>
                                            <p className="text-base font-bold text-foreground">
                                                {item.area}
                                            </p>
                                            <div className="flex items-center gap-1.5 mt-1">
                                                <AlertCircle className={`h-4 w-4 ${item.severity === 'high' ? 'text-red-500' : 'text-orange-400'}`} />
                                    ...
                                    <div className="pt-1">
                                        <span className={`text-xs font-bold px-2 py-1 rounded-md ${item.severity === 'high' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}>
                            ...
                            <div className="flex flex-col items-center justify-center py-8 text-center">
                                ...
                                <p className="text-muted-foreground font-medium">No major bottlenecks detected.</p>
                                <p className="text-xs text-muted-foreground/70">Great job keeping the workflow smooth!</p>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-8">
                        ...
                        <div className="flex-1 space-y-8">
                            {membersData.length > 0 ? (
                                ...
                                <p className="text-center text-muted-foreground py-8">No member performance data available.</p>
                            )}
                        </div>
                ...
                <div className="rounded-[2.5rem] bg-card p-8 border border-border">
                    ...
                    <div className="space-y-8">
                        <div className="flex justify-between items-center">
                            ...
                            <p className="text-base font-bold text-foreground">
                                {membersData.length > 0
                                    ? membersData.reduce((prev, current) => (prev.completed > current.completed) ? prev : current).name
                                    : "N/A"}
                            </p>
                        </div>
                        <div className="flex justify-between items-center">
                            ...
                            <p className="text-base font-bold text-foreground">
                                {membersData.length > 0
                                    ? membersData.reduce((prev, current) => (prev.overdue < current.overdue) ? prev : current).name
                                    : "N/A"}
                            </p>
                        </div>
                        <div className="flex justify-between items-center">
                            ...
                            {membersData.length === 0 ? (
                                ...
                                membersData.map((member, idx) => (
                                    <tr key={idx} className="group hover:bg-muted/30 transition-colors">
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-3">
                                                <UserAvatar
                                                    ...
                                                    size={40}
                                                    className="h-10 w-10 border border-border group-hover:border-blue-200 transition-colors"
                                                />
                                        ...
                                        <td className="px-6 py-5 font-bold text-muted-foreground">{member.assigned || 0}</td>
                                        <td className="px-6 py-5 font-bold text-green-500">{member.completed}</td>
                                        <td className="px-6 py-5 font-bold text-red-400">{member.overdue || 0}</td>
...
function PerformanceReportModal({ member, onClose, userRole }: { member: Member; onClose: () => void; userRole?: string }) {
    ...
    const handleDownloadIndividualReport = () => {
        ...
        link.setAttribute("href", url);
        link.setAttribute("download", `${member.name.replace(/ /g, '_')}_Performance_Report_${format(new Date(), 'yyyy-MM-dd')}.csv`);
        link.style.visibility = 'hidden';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="bg-muted rounded-3xl p-6 flex flex-col border border-border">
                            ...
                            <div className="mt-6 w-full space-y-3 text-left flex flex-col items-center">
                                ...
                                <p className="font-medium text-sm text-muted-foreground">{member.jobRole || "Team Member"}</p>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between items-end">
                                    ...
                                    <p className="text-green-600 font-bold text-lg">{member.rate || "0%"}</p>
                                </div>
                                <div className="h-16 w-full bg-muted rounded-xl overflow-hidden flex border border-border">
                                    <div
                                        ...
                                        style={{ width: member.efficiency ? `${member.efficiency}%` : '0%' }}
                                    />
                                </div>
                            </div>
