...
export default function NotificationsPage() {
    ...
    const workspaceUnreadCount = notifications.filter(n => !n.isRead && n.workspace === currentWorkspace?.name).length;
    ...
    const transformApiData = (apiNotifications: any[]): NotificationItem[] => {
        return apiNotifications.map(notification => {
            ...
            const workspace = workspaces.find(ws => ws.id === notification.workspace);
            ...
            return {
                ...
                context: notification.note_type === 'task_assigned' ? 'Tasks' :
                    notification.note_type === 'deadline_approaching' ? 'Tasks' :
                        notification.note_type?.includes('wiki') ? 'to Wiki' :
                            notification.note_type?.includes('link') ? 'to Quick Links' : 'Tasks',
                time: formatTimeAgo(notification.created_at),
                workspace: workspaceName,
                isRead: notification.is_read,
                type: notification.note_type === 'task_assigned' ? 'assign' :
                    ...
                    >
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 space-y-1">
                                ...
                                {(notification.type === "assign" || notification.type === "comment") && notification.content && (
